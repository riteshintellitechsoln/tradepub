...
export default async function AdminActivityPage({ searchParams }: AdminActivityPageProps) {
  ...
  const exportData = activity.map((row) => ({
    "Lead Name": row.leadName,
    "Lead Email": row.leadEmail,
    Book: row.bookTitle,
    Status: row.status,
    "Email Attempts": row.emailAttempts,
    "Latest Email Error": row.latestEmailError ?? "",
    Requested: format(row.createdAt, "yyyy-MM-dd HH:mm"),
    Downloaded: row.downloadedAt ? format(row.downloadedAt, "yyyy-MM-dd HH:mm") : "",
  }));

  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold">Activity ({totalCount})</h1>
        <ExportExcelButton data={exportData} filename="activity-export" sheetName="Activity" />
      </div>
