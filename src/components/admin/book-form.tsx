  ...
  const form = useForm<BookFormValues>({
    ...
    defaultValues: book
      ? {
          ...
          shortDescription: book.shortDescription ?? undefined,
          ...
          downloadLimit: book.downloadLimit ?? undefined,
          forceOutOfStock: book.forceOutOfStock,
          tags: book.tags.join(", "),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="max-w-3xl space-y-6" noValidate>
        ...
        <div className="space-y-4 rounded-lg border p-4">
          <FormField
            control={form.control}
            name="forceOutOfStock"
            render={({ field }) => (
              <FormItem>
                <FormLabel>Stock Mode</FormLabel>
                {/* <Select
                  onValueChange={(value) => field.onChange(value === "blocked")}
                  value={field.value ? "blocked" : "auto"}
                > */}

                                <Select
                  onValueChange={(value) => {
                    const isBlocked = value === "blocked";
                    field.onChange(isBlocked);
                    if (!isBlocked && form.getValues("downloadLimit") == null) {
                      form.setValue("downloadLimit", 999);
                    }
                  }}
                  value={field.value ? "blocked" : "auto"}
                >
                  ...
                  <SelectContent>
                    <SelectItem value="auto">By Default (Automatic)</SelectItem>
                    <SelectItem value="blocked">Block (Out of Stock now)</SelectItem>
                  </SelectContent>
                </Select>
                <FormDescription>
                  <span className="font-medium text-foreground">By Default</span> — stock
                  automatically tracks the download limit below.{" "}
                  <span className="font-medium text-foreground">Block</span> — instantly
                  marks this book Out of Stock, no matter how many downloads are left.
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
          ...
          {!form.watch("forceOutOfStock") && (
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    Download limit <span className="text-muted-foreground">(stock)</span>
                  </FormLabel>
                  <FormControl>
                    <Input
                      type="number"
                  ...
                  <FormDescription>
                    Once this many downloads happen, the book shows as &quot;Out of
                    Stock&quot; and can&apos;t be downloaded again until you raise or
                    clear this limit.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
